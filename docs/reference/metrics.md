...
| Interest Coverage | `interest_coverage` | EODHD-oriented: `EBIT_TTM / InterestExpense_TTM`, with rescue-only fallback from `interestIncome - netInterestIncome`. When TTM EBIT is positive and fresh but interest expense is absent, stale, misaligned, or non-positive, the metric emits the documented cap `100.0` instead of NA — but only with fresh no-material-debt evidence: `max(ShortTermDebt + LongTermDebt, TotalDebtFromBalanceSheet) <= 1.0x TTM EBIT` (falling back to total `Liabilities`, an upper bound on debt, when no debt concept is fresh). The 1.0x slack absorbs EODHD's lease/derived-liability contamination of debt fields (web-verified debt-free PLTR.US shows 0.29x). Material evidence, or no fresh balance-sheet facts at all, stays NA — a missing interest line on a levered issuer is a provider data gap, not debt-freedom (2026-07-06 audit: 55% of cap-path listings carried fresh debt above 1x TTM EBIT). The cap is a convention for an economically unbounded ratio, not a measurement; loss-making or stale-EBIT issuers stay NA. | Tests whether operating profit comfortably covers financing cost — without excluding the strongest (debt-free) balance sheets from `>=` screen gates, and without letting missing provider data manufacture a pass. |
