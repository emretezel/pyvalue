...
| ROIC 10Y Median | `roic_10y_median` | EODHD-oriented median of strict 10 FY ROIC values. Requires 10 consecutive ROIC years, which in practice means 11 consecutive FY invested-capital points. A year whose average invested capital is non-positive fails (the ratio is undefined at or below a zero capital base and would sign-flip) — it is never emitted as a point. | Captures central tendency of long-cycle capital efficiency. |
| ROIC 7Y Median | `roic_7y_median` | EODHD-oriented median of strict 7 FY ROIC values. Requires 7 consecutive ROIC years and therefore 8 consecutive FY invested-capital points. Non-positive average invested capital fails the year, as for `roic_10y_median`. | A shorter-horizon coverage alternative when the 10Y window is too sparse but you still want strict history. |
