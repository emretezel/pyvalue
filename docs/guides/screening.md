...
- [`screeners/quality_reasonable_price_primary.yml`](../../screeners/quality_reasonable_price_primary.yml): the primary QARP (quality at a reasonable price) screen; hard-gates durability plus reinvestment (`iroic_5y`), gross-margin level and stability, full-cycle earnings quality (`cfo_to_ni_10y_median`), and modernized Graham earnings stability, with valuation and debt service expressed as OR groups so a single-lens gate cannot over-exclude — "reasonable price" passes on any of owner-earnings / EBIT / FCF yield on EV, and "debt service" passes on 6x interest coverage or already-low (`<= 1.5x`) leverage — ranked sector-relative on a blend spanning quality/capital-efficiency, valuation, capital allocation, and earnings stability, with post-screen `qarp_score` and `qarp_rank` output for passing symbols
- [`screeners/deep_value_graham.yml`](../../screeners/deep_value_graham.yml): deep-value Graham screen; deliberately loose gates that exclude broken businesses rather than demand quality — bare structural floors (positive adaptive ROIC median, `cfo_to_ni_10y_median >= 0.60`, loss-year share, Piotroski F-Score >= 5, a USD 150M market-cap investability floor), a `>= 3-of-4` **solvency scorecard** (leverage, interest coverage, Altman Z, FCF/EBITDA) that tolerates one benign cyclical trip, and a valuation OR that is cheap on book, EV/EBIT, or FCF yield (so asset-light value is no longer excluded by P/B alone) — ranked sector-relative on a cheapness-weighted blend of valuation (`price_to_book`, `ev_to_sales`, `ev_to_ebit`), capped capital efficiency (`croic`, `roce`), and composite health (`piotroski_f_score`, `altman_z`)
