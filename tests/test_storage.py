...
def _stored_lei(db_path: Path, symbol: str, exchange_code: str) -> Optional[str]:
    """Read ``listing.lei`` for one canonical listing."""

    with sqlite3.connect(db_path) as conn:
        row = conn.execute(
            """
            SELECT l.lei FROM listing l
            JOIN "exchange" e ON e.exchange_id = l.exchange_id
            WHERE l.symbol = ? AND e.exchange_code = ?
            """,
            (symbol, exchange_code),
...
def test_ingest_persists_listing_lei_from_the_payload(tmp_path: Path) -> None:
    """``General.LEI`` is the only source for ``listing.lei``.

    The provider's symbol list does not carry it, so unlike ``isin`` the catalog
    refresh cannot populate it. Without this write the column would only ever
    hold what migration 088 backfilled and would decay as the catalog grew --
    quietly degrading issuer identity.
    """

    db_path = tmp_path / "ingest-lei.db"
    ticker_repo = SupportedTickerRepository(db_path)
    ticker_repo.initialize_schema()
    seed_exchange(db_path, "US")
    ticker_repo.replace_for_exchange(
        "EODHD",
        "US",
        [{"Code": "MTD", "Name": "Mettler", "Type": "Common Stock", "Currency": "USD"}],
    )
    ...
    seed_raw_fundamentals(
        ...
        "MTD.US",
        {
            "General": {
                "Name": "Mettler-Toledo",
                "PrimaryTicker": "MTD.US",
                "LEI": "5493000BD5GJNUDIUG10",
            }
        },
    )

    assert _stored_lei(db_path, "MTD", "US") == "5493000BD5GJNUDIUG10"


def test_ingest_never_clears_a_stored_lei(tmp_path: Path) -> None:
    """A payload omitting the field is a provider gap, not a retraction."""

    db_path = tmp_path / "ingest-lei-retain.db"
    ticker_repo = SupportedTickerRepository(db_path)
    ticker_repo.initialize_schema()
    seed_exchange(db_path, "US")
    ticker_repo.replace_for_exchange(
    ...
    seed_raw_fundamentals(
        ...
        "EODHD",
        "MTD.US",
        {"General": {"PrimaryTicker": "MTD.US", "LEI": "5493000BD5GJNUDIUG10"}},
    )
    seed_raw_fundamentals(
        db_path,
        "EODHD",
        "MTD.US",
        {"General": {"PrimaryTicker": "MTD.US"}},
    )

    assert _stored_lei(db_path, "MTD", "US") == "5493000BD5GJNUDIUG10"


def test_ingest_ignores_a_malformed_lei(tmp_path: Path) -> None:
    """Shape-checked at the boundary so the column CHECK is never tripped."""

    db_path = tmp_path / "ingest-lei-malformed.db"
    ticker_repo = SupportedTickerRepository(db_path)
    ticker_repo.initialize_schema()
    seed_exchange(db_path, "US")
    ticker_repo.replace_for_exchange(
    ...
    seed_raw_fundamentals(
        ...
        "EODHD",
        "MTD.US",
        {"General": {"PrimaryTicker": "MTD.US", "LEI": "not-an-lei"}},
    )

    assert _stored_lei(db_path, "MTD", "US") is None
