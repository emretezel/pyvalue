...
class ROCGreenblattMetric:
    ...
    def compute(
        ...
        ebit_records = self._fetch_ebit_history(listing_id, repo)
        if not ebit_records:
            LOGGER.warning(
                "roc_greenblatt: no FY EBIT records for listing_id=%s", listing_id
            )
            return None
        if not has_recent_fact(
            repo, listing_id, EBIT_CONCEPTS, max_age_days=MAX_FY_FACT_AGE_DAYS
        ):
            LOGGER.warning(
                "roc_greenblatt: no recent FY EBIT fact for listing_id=%s", listing_id
            )
            return None
        ...
        tc_map = self._fetch_tangible_capital(listing_id, repo, target_currency)
        if not tc_map:
            LOGGER.warning(
                "roc_greenblatt: missing tangible capital components for listing_id=%s",
                listing_id,
            )
            return None
        assets_check = repo.latest_monetary_fact(listing_id, "AssetsCurrent")
        liabilities_check = repo.latest_monetary_fact(listing_id, "LiabilitiesCurrent")
        if assets_check is None or not is_recent_fact(
            assets_check, max_age_days=MAX_FY_FACT_AGE_DAYS
        ):
            LOGGER.warning(
                "roc_greenblatt: no recent assets current for listing_id=%s", listing_id
            )
            return None
        if liabilities_check is None or not is_recent_fact(
            ...
            LOGGER.warning(
                "roc_greenblatt: liabilities current too old for listing_id=%s",
                listing_id,
            )
            return None
        ...
        if not values:
            LOGGER.warning(
                "roc_greenblatt: insufficient overlapping years for listing_id=%s",
                listing_id,
            )
            return None
        ...
        return MetricResult(
            listing_id=listing_id, metric_id=self.id, value=avg, as_of=latest
        )
    ...
    def _fetch_ebit_history(
        ...
        return repo.monetary_facts_for_concept(
            listing_id, "OperatingIncomeLoss", fiscal_period="FY"
        )
    ...
    def _fetch_tangible_capital(
        ...
        ppe_records = repo.monetary_facts_for_concept(
            listing_id, "PropertyPlantAndEquipmentNet", fiscal_period="FY"
        )
        if not ppe_records:
            return {}
        assets_by_period = self._index_by_period(
            repo.monetary_facts_for_concept(
                listing_id, "AssetsCurrent", fiscal_period="FY"
            )
        )
        liabilities_by_period = self._index_by_period(
            repo.monetary_facts_for_concept(
                listing_id, "LiabilitiesCurrent", fiscal_period="FY"
            )
        )
        ...
        for ppe in ppe_records:
            ...
            if assets is None or liabilities is None:
                continue
            tangible_capital = (
                self._money(ppe, target_currency, listing_id)
                + self._money(assets, target_currency, listing_id)
                - self._money(liabilities, target_currency, listing_id)
            )
