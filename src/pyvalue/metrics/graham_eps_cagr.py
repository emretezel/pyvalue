...
def compute_graham_eps_cagr(
    listing_id: int, repo: RegionFactsRepository, *, context: str
) -> Optional[tuple[float, str]]:
    """Resolve the Graham smoothed EPS growth rate: ``(cagr, as_of)``.

    The growth-rate core of ``graham_eps_10y_cagr_3y_avg``, shared with the
    PEG metrics: 3-year-averaged endpoints over the latest 10 FY EPS window,
    compounded over the ``CAGR_YEARS`` separating the endpoint midpoints.
    ``context`` tags log lines with the calling metric id. Returns ``None``
    when fewer than 10 FY EPS years exist, the latest FY EPS is stale, or an
    endpoint average is non-positive.
    """
    ...
    if len(records) < MIN_REQUIRED:
        LOGGER.warning(
            "%s: need >=%s FY EPS records for listing_id=%s, found %s",
            context,
            MIN_REQUIRED,
            listing_id,
            len(records),
    ...
    if not has_recent_fact(
        ...
        LOGGER.warning(
            "%s: no recent FY EPS fact for listing_id=%s", context, listing_id
        )
        return None
    ...
    if len(ordered) < MIN_REQUIRED:
        LOGGER.warning(
            "%s: need >=%s unique FY EPS records for listing_id=%s after filtering, found %s",
            context,
            MIN_REQUIRED,
            listing_id,
            len(ordered),
    ...
    cagr_value = _compute_cagr(eps_amounts)
    if cagr_value is None:
        LOGGER.warning(
            "%s: could not derive CAGR value for listing_id=%s", context, listing_id
        )
        return None
    return cagr_value, latest_date


def _compute_cagr(eps_amounts: List[float]) -> Optional[float]:
...
class GrahamEPSCAGRMetric:
    ...
    def compute(
        ...
        resolved = compute_graham_eps_cagr(listing_id, repo, context=self.id)
        if resolved is None:
            return None
        cagr_value, latest_date = resolved
        return MetricResult(
            listing_id=listing_id,
            metric_id=self.id,
            value=cagr_value,
