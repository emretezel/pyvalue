...
- When reviewing metric calculations, verify that every monetary input is denominated in the same currency as the listing's currency. If any input is in a different currency, do not silently FX-convert it — raise an exception and/or report the mismatch to the user so the underlying data issue can be addressed.
